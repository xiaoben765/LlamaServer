...
private:
    ...
    void handleChatRequest(const HttpRequest& req, HttpResponse& resp) {
        try {
            ...
            if (lfu_cache_.get(message, cachedResult)) {
                LOG_INFO << "返回缓存结果";
                resp.setJsonResponse(buildJsonResponse(cachedResult, true));
                return;
            }
            ...
            while (retryCount < 3 && !success) {
                ...
                try {
                    result = llama_service_.query(message);
            ...
            if (success) {
                ...
                resp.setJsonResponse(buildJsonResponse(result, false));
            } else {
                // 所有尝试均失败
                ...
                resp.setJsonResponse(buildJsonResponse("LLaMA服务暂时不可用，请稍后重试", false));
            }
        } catch (const std::exception& e) {
    ...
    void handleStatusRequest(const HttpRequest& req, HttpResponse& resp) {
        resp.setJsonResponse(buildJsonStatus(
            static_cast<int>(lfu_cache_.size()),
            llama_service_.isServiceAvailable()
        ));
    }
    ...
    std::string getWebInterface() {
        ...
        html += "        .input-area { display: flex; gap: 10px; }\n";
        ...
        html += "        }\n";
        html += "\n";
        html += "        function updateStatus(text) {\n";
        html += "            document.getElementById('status').textContent = text;\n";
        html += "        }\n";
        html += "\n";
        ...
        html += "                body: JSON.stringify({ message: message })\n";
        html += "            })\n";
        ...
        html += "            .then(response => response.json())\n";
        html += "            .then(data => {\n";
        html += "                updateStatus(`缓存: ${data.cache_size} | LLaMA: ${data.llama_available ? '可用' : '不可用'}`);\n";
        html += "            })\n";
        html += "            .catch(() => updateStatus('状态获取失败'));\n";
        html += "    </script>\n";
        html += "</body>\n";
        html += "</html>\n";
    ...
    // 添加会话管理
    std::map<std::string, std::vector<std::string>> chatHistory;
    
    // 会话超时设置（可选）
    const int SESSION_TIMEOUT = 30 * 60; // 30分钟
};
